...
const TOOL_NAME = "resolve_legal_entity";
const ACTOR_ID = "KHFyPCDIx7CyqULYm";
...
const ACTOR_INPUTS = ["domain", "jurisdiction_hint", "legal_name_hint", "match_strictness", "skipCache", "validate_vat"];
...
test("package identity matches the locked naming convention", () => {
  ...
  assert.deepEqual(key, ["mamba-legal-entity-resolver"]);
  assert.deepEqual(mcp.mcpServers[key[0]].args, ["-y", pkg.name]);
  assert.equal(pkg.name, "@mambalabsdev/mcp-legal-entity-resolver");
  assert.equal(pkg.mcpName, "com.mambabuilt/mcp-legal-entity-resolver");
});
